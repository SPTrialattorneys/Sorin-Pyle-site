...
console.log(`${colors.blue}🔍 Pre-Commit Validation${colors.reset}\n`);
...
function checkDistFiles() {
  ...
  try {
    ...
    if (distFiles.length > 0) {
      console.log(`${colors.red}✗ Found ${distFiles.length} dist/ files staged for commit:${colors.reset}`);
    ...
    console.log(`${colors.green}✓ No dist/ files staged${colors.reset}\n`);
    return true;
  } catch (error) {
    console.log(`${colors.yellow}⚠ Could not check staged files (not a git repo?)${colors.reset}\n`);
    return true;
  }
}
...
function checkConsoleLogs() {
  ...
  try {
    ...
    if (foundConsoleLogs.length > 0) {
      console.log(`${colors.yellow}⚠ Found ${foundConsoleLogs.length} console.log statement(s):${colors.reset}`);
      foundConsoleLogs.forEach(item => {
        console.log(`  ${colors.yellow}•${colors.reset} ${item.file}:${item.line}`);
    ...
    console.log(`${colors.green}✓ No console.log found in production JS${colors.reset}\n`);
    return true;
  } catch (error) {
    console.log(`${colors.gray}○ Skipped (no staged JS files)${colors.reset}\n`);
    return true;
  }
}
...
function checkTodoComments() {
  ...
  try {
    ...
    if (foundTodos.length > 0) {
      console.log(`${colors.yellow}⚠ Found ${foundTodos.length} TODO comment(s):${colors.reset}`);
    ...
    console.log(`${colors.green}✓ No TODO comments found${colors.reset}\n`);
    return true;
  } catch (error) {
    console.log(`${colors.gray}○ Skipped (no staged .njk files)${colors.reset}\n`);
    return true;
  }
}
...
function validateSchema() {
  ...
  try {
    ...
    if (!hasSchemaFiles) {
      console.log(`${colors.gray}○ Skipped (no template files staged)${colors.reset}\n`);
      return true;
    }
  } catch (error) {
...
function main() {
  ...
  if (hasErrors) {
    console.log(`${colors.red}✗ Pre-commit validation FAILED${colors.reset}`);
    ...
    console.log(`${colors.yellow}⚠ Pre-commit validation passed with warnings${colors.reset}`);
